...
authentificate() {
  clientId="${1}"
  clientSecret="${2}"
  accessToken=$(curl -s -u ${clientId}:${clientSecret} -d grant_type=client_credentials "https://${region}.battle.net/oauth/token" | jq -r '.access_token')
...
wowItem() {
  curl -s -H "Authorization: Bearer ${accessToken}" \
    ...
    | jq '{id: .id, name: .name, item_class: .item_class | {id: .id, name: .name}, item_subclass: .item_subclass | {id: .id, name: .name}, inventory_type: .inventory_type, preview_item: .preview_item | {name_description_display_string: .preview_item.name_description.display_string, spells_description: .spells[].description}}';
}
